...
use serde::Serialize;
use slimchain_chain::{
    behavior::TxExecuteStream, config::ChainConfig, consensus::pow::Block, db::DBPtr,
    latest::LatestTxCount, role::Role,
};
...
impl<Tx: TxTrait + Serialize + 'static> StorageBehavior<Tx> {
    ...
    fn poll_inner<T>(
        ...
        if let Poll::Ready(Some(tx_proposal)) = Pin::new(&mut self.tx_exec_stream).poll_next(cx) {
            self.pubsub
                .publish_tx_proposal(&tx_proposal.tx)
...
impl<Tx: TxTrait + Serialize>
    NetworkBehaviourEventProcess<PubSubEvent<Tx, BlockProposal<Block, Tx>>>
    for StorageBehavior<Tx>
{
