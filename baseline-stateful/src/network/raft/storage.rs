...
impl<Tx: TxTrait + Serialize> SendToLeader<Tx> {
    ...
    async fn send_txs(&self, txs: &Vec<Tx>) -> Result<()> {
        ...
        match send_reqs_to_leader(leader_addr, txs).await {
            Err(e) => {
...
impl TxExecWorker {
    fn new<Tx: TxTrait + Serialize>(
        ...
        let tx_exec_fut = TxExecuteStream::new(tx_req_rx, engine, &db, &latest_block_header)
            .map(|tx_proposal| tx_proposal.tx)
            .ready_chunks(8)
            .for_each_concurrent(8, move |txs| {
                let send_to_leader = send_to_leader.clone();
                async move {
                    for i in 1..=MAX_RETRIES {
                        match send_to_leader.send_txs(&txs).await {
                            Ok(_) => break,
                            Err(e) => {
                                if i == MAX_RETRIES {
                                    error!(
                                        "Failed to send tx to raft leader. Error: {}",
                                        e
                                    );
                                    for tx in &txs {
                                        let tx_id = tx.id();
                ...
                tokio::select! {
                    ...
                    Some(blk_proposal) = blk_rx.next() => {
                        let state_update = {
                            ...
                            match verify_block(&chain_cfg, &db, &mut snapshot, &blk_proposal, verify_consensus)
                                .await
                            {
