use crate::{block_proposal::BlockProposal, network::raft::message::NewBlockRequest};
use async_raft::{
    raft::{
...
use slimchain_chain::{consensus::raft::Block, role::Role};
    ...
    Tx: TxTrait + Serialize + for<'de> Deserialize<'de> + 'static,
{
    ...
    pub async fn forward_tx_to_storage_node(&self, tx_req: TxHttpRequest) {
        ...
        let storage_node_peer_id = match self
            .route_table
            .random_peer(&Role::Storage(ShardId::default()))
        {
            Some(peer) => peer,
            None => {
                error!(%tx_req_id , "Failed to find the storage node.");
                return;
            }
        };
    ...
    #[tracing::instrument(level = "debug", skip(self, txs), err)]
    pub async fn forward_tx_to_leader(&self, txs: &Vec<Tx>) -> Result<()> {
        ...
        match send_reqs_to_leader(addr, txs).await {
            Err(e) => {
