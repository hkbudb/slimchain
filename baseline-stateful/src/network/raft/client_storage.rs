...
pub struct ClientNodeStorage<Tx: TxTrait> {
    ...
    miner_snapshot: Mutex<Option<(H256, Snapshot<Block>, TxStateUpdate)>>,
    ...
    Tx: TxTrait + Serialize + for<'de> Deserialize<'de> + 'static,
{
    ...
    pub async fn get_block_proposal(
        ...
        BlockProposal::from_existing_block(block, &self.db)
    }
    ...
    pub async fn latest_snapshot(&self) -> Snapshot<Block> {
    ...
    pub async fn set_miner_snapshot(
        ...
        *miner_snapshot = Some((blk_hash, snapshot, state_update));
    }
    ...
    Tx: TxTrait + Serialize + for<'de> Deserialize<'de> + 'static,
{
    ...
    async fn apply_entry_to_state_machine(
        ...
        let mut miner_snapshot = self.miner_snapshot.lock().await;
        if let Some((blk_hash, m_snapshot, m_state_update)) = miner_snapshot.take() {
            if blk_proposal.get_block().to_digest() == blk_hash {
                snapshot = m_snapshot;
                state_update = m_state_update;
                miner = true;
            }
        }
    ...
    async fn finalize_snapshot_installation(
...

        {
            ...
            let mut height = sm.snapshot.current_height();
            while height < new_snapshot.snapshot.current_height() {
                let block_proposal =
                    get_block_proposal::<Tx>(leader_addr, height.next_height()).await?;
                let state_update = verify_block(
                    &self.chain_cfg,
                    &self.db,
                    &mut sm.snapshot,
                    &block_proposal,
            ...
            ensure!(sm.snapshot == new_snapshot.snapshot, "inconsistent block");
