...
use slimchain_network::{
    behavior::raft::{
        storage::{fetch_leader_id, LEADER_ID_CACHE_TTL, MAX_RETRIES},
        utils::LeaderPeerIdCache,
    },
    http::{
...
pub struct ClientNodeNetwork {
    ...
    leader_id_cache: Mutex<LeaderPeerIdCache>,
}
...
impl ClientNodeNetwork {
    pub fn new(route_table: NetworkRouteTable) -> Self {
        Self {
            route_table,
            leader_id_cache: Mutex::new(LeaderPeerIdCache::new()),
        }
    }
        ...
        'outer: for tx_req in tx_reqs {
            ...
            for i in 1..=MAX_RETRIES {
                match self.forward_tx_reqs_to_leader(&reqs).await {
                    Ok(_) => continue 'outer,
                    Err(e) => {
                        self.leader_id_cache.lock().await.reset();
                        if i == MAX_RETRIES {
